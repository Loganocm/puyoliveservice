...
export function OnboardingScreen({ onComplete }: OnboardingScreenProps) {
  ...
  return (
    <div className="size-full relative overflow-hidden bg-[#0a0a12] flex items-center justify-center font-sans pointer-events-auto">
      {/* Background effects */}
      <div 
      ...
      >
        ...
        <div className="text-center mb-10">
            <h1 className="text-4xl font-black text-white mb-2 tracking-tight">
                {phase === 'identity' ? "Puyo Live" : (phase === 'login' ? "Welcome Back" : "Join Puyo Live")}
            </h1>
            <p className="text-white/50 font-medium">
                {phase === 'identity' && "Enter a username to begin"}
                {phase === 'login' && `Log in to ${username}`}
                {phase === 'register' && `Create account for ${username}`}
            </p>
            {/* Legal Disclaimer */}
            <p className="text-white/30 text-xs mt-4 max-w-xs mx-auto leading-relaxed">
                By continuing, you agree to our{' '}
                <a href="/privacy" target="_blank" rel="noopener noreferrer" className="underline hover:text-white/50 transition-colors">
        ...
        <div className="space-y-4">
                ...
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
                    <div className="relative">
                        ...
                        <input
                            ...
                            onKeyDown={(e) => e.key === 'Enter' && (phase === 'login' ? handleLogin() : null)}
            ...
            {/* Error Message */}
            {error && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="bg-red-500/10 border border-red-500/20 rounded-lg p-3">
            ...
            <button
                ...
                className={`w-full py-4 rounded-xl font-black text-white text-base tracking-wider uppercase relative overflow-hidden transition-all transform active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed ${
                    phase === 'identity' && !username.trim() ? "bg-white/5 hover:bg-white/10" : "bg-gradient-to-r from-indigo-600 to-violet-600 hover:shadow-lg hover:shadow-indigo-500/20"
                }`}
            >
                {isLoading ? (
                    ...
                    <span>
                        {phase === 'identity' && (!username.trim() ? "Play as Guest" : "Continue")}
                        {phase === 'login' && "Log In"}
