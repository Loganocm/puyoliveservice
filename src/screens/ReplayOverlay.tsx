...
export const ReplayOverlay: React.FC<ReplayOverlayProps> = ({ onExit }) => {
    ...
    const [totalFrames, setTotalFrames] = useState(1); // Avoid div/0
    const [speed, setSpeed] = useState(1);
    const [uiVisible, setUiVisible] = useState(true);
    const hideTimeoutRef = useRef<ReturnType<typeof setTimeout>>();

    useEffect(() => {
        const handleUpdate = (data: { currentFrame: number, totalFrames: number, isPaused: boolean, speed: number }) => {
        ...
        const handleKeyDown = (e: KeyboardEvent) => {
            if (e.code === 'Space') {
                ...
                e.preventDefault();
                sendControl('seek', Math.min(totalFrames, currentFrame + 180)); // +3s
            } else if (e.code === 'ArrowLeft') {
                e.preventDefault();
                sendControl('seek', Math.max(0, currentFrame - 180)); // -3s
    ...
    const sendControl = (action: 'play' | 'pause' | 'seek' | 'speed' | 'exit', value?: number) => {
    ...
    const handleSeek = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setCurrentFrame(frame); // Optimistic update
    ...
    // Format frames to time (assuming 60fps)
    ...
    return (
        <div className={`fixed inset-0 pointer-events-none transition-opacity duration-300 ${uiVisible ? 'opacity-100' : 'opacity-0'}`}>
            {/* Header / Top Bar */}
            <div className="absolute top-0 left-0 w-full p-6 flex justify-between items-start pointer-events-auto bg-gradient-to-b from-black/80 to-transparent">
                <div>
                   <h2 className="text-white text-2xl font-bold tracking-wider font-mono">REPLAY VIEWER</h2> 
            ...
            <div className="absolute bottom-0 left-0 w-full p-8 pb-12 pointer-events-auto bg-gradient-to-t from-black/90 via-black/60 to-transparent">
                <div className="max-w-4xl mx-auto flex flex-col gap-4">
                    ...
                    <div className="flex items-center gap-4">
                        <span className="text-white font-mono w-16 text-right">{formatFrameTime(currentFrame)}</span>
                        ...
                        <span className="text-white/60 font-mono w-16">{formatFrameTime(totalFrames)}</span>
                    </div>
                    ...
                    <div className="flex justify-between items-center px-4">
                         ...
                         <div className="flex items-center gap-2">
                            <span className="text-white/40 text-xs font-bold uppercase mr-2">Speed</span>
                            {[0.5, 1, 2, 4].map(s => (
                                <button
                                    key={s}
                         ...
                         <div className="flex items-center gap-6">
                            <button onClick={() => sendControl('seek', Math.max(0, currentFrame - 300))} className="text-white/60 hover:text-white transition-colors">
                                <Rewind size={24} />
                            </button>
                            ...
                            >
                                {isPlaying ? <Pause fill="black" size={32} /> : <Play fill="black ml-1" size={32} />}
                            </button>

                            <button onClick={() => sendControl('seek', Math.min(totalFrames, currentFrame + 300))} className="text-white/60 hover:text-white transition-colors">
                                <FastForward size={24} />
                            </button>
                         </div>
